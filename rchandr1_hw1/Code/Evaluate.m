...
results = [mean(mean(abs(cameraParams.IntrinsicMatrix' - K_opt))),mean(mean(abs(cameraParams.TranslationVectors' - reshape(ts_opt, [3, size(ts_opt, 3)])))),mean(mean(mean(abs(permute(cameraParams.RotationMatrices, [2 1 3]) - Rs_opt)))),mean(mean(abs(cameraParams.RadialDistortion(:) - ks_opt(:))))]
